...
const resumeData = ref<HTMLElement | null>(null);

const validatePixGeneration = () =>{
    console.log('clique');
    pixDataArea.value.pixValidationErrors='';
    if(pixDataArea.value.pixKey == ''){
        console.log('pixDataArea.pixKey');
        pixDataArea.value.pixValidationErrors = 'Utilize uma chave pix válida';
    }else if(parseFloat(pixDataArea.value.pixValue.toString().replace(',','.')) <= parseFloat('0,01'.replace(',','.'))){
        console.log('pixDataArea.pixValue');
        pixDataArea.value.pixValidationErrors = 'Utilize um valor válido para o pix. \nDeve ser maior que 0,01';
    }else{
        if (resumeData.value !== null) {
            resumeData.value.innerHTML = `
        Emissor: ${pixDataArea.value.pixKey}<br />
        Valor: ${pixDataArea.value.pixValue.toString()}<br />
        ${pixDataArea.value.pixDescription!=''? 'Descrição: '+pixDataArea.value.pixDescription+'<br />':''}
        ...
        <div class="flex h-full flex-1 flex-row items-center gap-4 rounded-xl p-4 overflow-x-auto">
            <div class="grid h-full gen-pc-container auto-rows-min gap-4 md:grid-cols-3">
                ...
                <div
                    ...
                    <div data-v-9cb1914c=""
                        class="md:col-span-2 p-4 md:p-0 shadow-md rounded-2xl bg-white">
                        <div data-v-9cb1914c="" class="flex flex-col justify-center items-center">
                            ...
                            <div data-v-9cb1914c="" class="w-full md:w-1/2"><label data-v-9cb1914c=""
                                    ...
                                    <div data-v-9cb1914c="" class="label"><span data-v-9cb1914c=""
                                        ...
                                        <option data-v-9cb1914c="" selected value="email">Email</option>
                                    ...
                                    <input data-v-9cb1914c="" :value="pixDataArea.pixKey" type="email" placeholder="Informe o email" maxlength="50"
                                    ...
                                    <div data-v-9cb1914c="" class="label"><span data-v-9cb1914c=""
                                            class="label-text font-semibold -mb-1 text-start">Identificação</span><span
                                            data-v-9cb1914c="" class="text-xs italic -mb-1">Opcional</span></div>
                                        <Textarea v-model="pixDataArea.pixDescription" placeholder="Taxa cobrança reunião" rows="5" cols="25" 
                                        class="input-sm rounded-md focus:border-2 focus:border-gray-900 border-[0.5px] border-gray-400 focus:outline-none w-full max-w-xs" />
                                </label><label data-v-9cb1914c="" class="mt-3 form-control w-full max-w-xs">
                                    <div data-v-9cb1914c="" class="label"><span data-v-9cb1914c=""
                                            class="label-text font-semibold -mb-1">Valor (R$)</span><span
                                            data-v-9cb1914c="" class="text-xs italic -mb-1">Opcional</span></div>
                                        <InputNumber placeholder="0,00" v-model="pixDataArea.pixValue" inputId="pixValue" :minFractionDigits="2" :maxFractionDigits="5" fluid />
                                </label></div>
                                <div><span class="font-bold text-red-500">{{ pixDataArea.pixValidationErrors }}</span></div>
                            <div data-v-9cb1914c=""><img data-v-9cb1914c=""
                                ...
                                </svg> Gerar Preview Pix </button>
                        </div>
                    </div>
                </div>
                <!-- <div class="flex justify-center flex-col min-w-[15%]"> -->
                    <div
                        class="max-h-fit status-card relative aspect-video rounded-xl border border-sidebar-border/70 dark:border-sidebar-border" ref="resumeData">
                        Resumo Pix
                    </div>
                <!-- </div> -->
                <div class="max-w-[10%] max-h-[10%]" >
                    <img src="https://res.cloudinary.com/dv33g6nco/image/upload/v1753783725/pngwing.com_ffqmso.png" alt="seta-orientation">
                </div>
                 </div>
                <Divider />
                <div class="w-full flex flex-row items-center justify-evenly">
                    ...
                    <div>
                        ...
                        <ModalDialog :usersToSend="selectedUsers" :pixData="pixDataArea"/>
                    </div>
                </div>
            </div>
        </div>
